class DebtsController < ApplicationController
  ...
  def sorted_debts
    ...
    if current_user.preferred_payoff_method == "snowball"
      credit_card_debts = payoff_debts.order(:total_balance).where(user_id: current_user.id, debt_type: "Credit Card")
    elsif current_user.preferred_payoff_method == "avalanche"
      credit_card_debts = payoff_debts.order(apr: :desc).where(user_id: current_user.id, debt_type: "Credit Card")
    end
    non_credit_card_debts = payoff_debts.where(user_id: current_user.id) && payoff_debts.where.not(debt_type: "Credit Card")
